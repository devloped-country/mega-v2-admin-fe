...
function ContentHeader({ title, img, classes, buttonText, buttonImg, isShowingButton, onButtonAction }) {
  const [isShowingSelect, setIsShowingSelect] = useState(false);
  const [selectedClassName, setSelectedClassName] = useState(classes && classes[0].name);
  ...
  const mapedClasses =
    classes &&
    classes.map((v, i) => (
      <li key={i} className={styles.option} data-tag="classSelect" onClick={() => handleClickOption(v)}>
        {v.name}
      </li>
    ));
