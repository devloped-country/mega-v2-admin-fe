...
import { addManagerReducer, initialAddManagerState } from "@/reducer/addManagerReducer";
import { useMutation } from "@/hooks/useMutation";
import axios from "axios";

function SettingManagerModal({ title, onClose }) {
  const [addManagerState, dispatch] = useReducer(addManagerReducer, initialAddManagerState);
  ...
  const onInvite = () => {
    const { name, email, phoneNumber, password, passwordConfirm } = addManagerState;

    if (!name.length || !email.length || !phoneNumber.length || !password.length || !passwordConfirm.length || password !== passwordConfirm) {
      return;
    }
